		...
		try {
			...
			for (Entry<String, long[]> entry : data.entrySet()) {
				...
				if(or_cost.size()>0)
				{
					...
					String costStr = or_cost.get(0).get("cost");
					if(!StringUtils.isStrEmpty(costStr))
					{
						try {
				...
				if(cost == -1)
				{
					cost = this.getCost(outFirstGetPer, outRegister, rewardId);
					costType = 0;
				}
				else {
					costType = 1;
				...
				try {
					...
					{
						//保存最后一次优化比例
						String sql2 = "update operate_data_log set optimization="+proportion+" where channelId= "+channelId+" and date = '"+date+"' ";
						int yx = od.Update(sql2);
						if(yx==0)
						{
							sql2 = "insert into operate_data_log(optimization,channelId,date) values("+proportion+" ,"+channelId+" , '"+date+"') ";
							yx = od.Update(sql2);
						}
					}
