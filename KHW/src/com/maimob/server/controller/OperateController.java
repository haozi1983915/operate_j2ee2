...
import com.maimob.server.controller.logic.AppmsgLogic;
...
public class OperateController extends BaseController {
			...
			if (!StringUtils.isStrEmpty(id)) {
				...
				if (!StringUtils.isStrEmpty(appid)) {
					...
					for (String app : appids) {
						List<Reward> rewardList2 = getReward(
								dao.findRewardByChannelId(Long.parseLong(channelId), Long.parseLong(app)));
		...
		if (!json.equals("")) {
			...
			if (!StringUtils.isStrEmpty(channelId)) {
				if (!StringUtils.isStrEmpty(appid)) {
					String[] appids = appid.split(",");
					for (String app : appids) {
						List<Reward> rewardList2 = getRewardMain(
								dao.findRewardByChannelId(Long.parseLong(channelId), Long.parseLong(app)));
	...
	public String checkflag(HttpServletRequest request, HttpServletResponse response) {
		...
		// + request.getContextPath() + "/page/forgrtPaw.html?email=" + email +"&res=" +
		// res + "&createurlTime" + ;
	...
	public String changePwd(HttpServletRequest request, HttpServletResponse response) {
		...
		// long currentTime = System.currentTimeMillis();
		// String path = request.getScheme() + "://" + request.getServerName() + ":" +
		// request.getServerPort() + request.getContextPath() + "/page/index.html";
		String path = "";
		if (type == 1) {
...
//		List<String> listName = Arrays.asList(str.split(","));
	...
	// 日报历史 运营数据下载
	// @RequestMapping(value = "/getBdAndMarketData", method = RequestMethod.POST,
	// produces = "text/html;charset=UTF-8")
	// @CrossOrigin(origins="*",maxAge=3600)
	// @ResponseBody
	// public String exportBdAndMarketData(HttpServletRequest
	...
	@RequestMapping(value = "/getMsgway", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@CrossOrigin(origins = "*", maxAge = 3600)
	@ResponseBody
	public String getMsgway(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.getMsgway(json);
	}

	@RequestMapping(value = "/saveMsgway", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@CrossOrigin(origins = "*", maxAge = 3600)
	@ResponseBody
	public String saveMsgway(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.saveMsgway(json);
	}

	@RequestMapping(value = "/saveAllMsgway", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@CrossOrigin(origins = "*", maxAge = 3600)
	@ResponseBody
	public String saveAllMsgway(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.saveAllMsgway(json);
	}
	...
	public String getMsg(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.getMsg(json);
	}

	@RequestMapping(value = "/saveMsg", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@CrossOrigin(origins = "*", maxAge = 3600)
	@ResponseBody
	public String saveMsg(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.saveMsg(json);
	}
	
	

	@RequestMapping(value = "/getMsgList", method = RequestMethod.POST, produces = "text/html;charset=UTF-8")
	@CrossOrigin(origins = "*", maxAge = 3600)
	@ResponseBody
	public String getMsgList(HttpServletRequest request, HttpServletResponse response) {
		String json = this.checkParameter(request);
		AppmsgLogic logic = new AppmsgLogic(dao);
		return logic.getMsgList(json);
	}
