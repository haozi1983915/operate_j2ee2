...
*/
	...
	public void exportExcelString(String title, List<String> headersName, List<String> headersId,
            List<Map<String, String>> dtoList,HttpServletResponse response) {
			/*
			（一）表头--标题栏
			*/
			Map<Integer, String> headersNameMap = new HashMap<>();
...
*/
    ...
    （四）导出数据：包括导出标题栏以及内容栏
			...
			//表格一行的字段的集合，以便拿到迭代器
	        Collection zdC = titleFieldMap.values();
	        Iterator<Map<String, String>> titleFieldIt = dtoList.iterator();//总记录的迭代器
	        int zdRow = 1;//真正的数据记录的列序号
	        while (titleFieldIt.hasNext()) {//记录的迭代器，遍历总记录
	            Map<String, String> mapTemp = titleFieldIt.next();//拿到一条记录
	                row = sheet.createRow(zdRow);
	                zdRow++;
	                int zdCell = 0;
	                Iterator<String> zdIt = zdC.iterator();//一条记录的字段的集合的迭代器
